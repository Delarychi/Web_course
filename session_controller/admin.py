from django.contrib import admin #файл это 9,10 пункты
...
class EvaluatorInline(admin.TabularInline): # позволяет внутри сессии добавлять связь
    model = Evaluator
    extra = 1
    fields = ('evaluator',)
    fk_name = 'session'
...
admin.site.register(Session, SessionAdmin) #самая важная херня(обычно тупо ее пишут)

class AssessmentResource(resources.ModelResource): #9 пункт
    class Meta:
        model = Assessment
